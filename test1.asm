...
; ===== SHOPPING CART =====
Cart DB 50 DUP(0)              ; Store selected items (max 50)
...
SearchMenu DB 0DH, 0AH, '=== PRODUCT SEARCH ===', 0DH, 0AH,
           DB '1 - Search by Product Code', 0DH, 0AH,
           DB '2 - Search by First Letter', 0DH, 0AH,
           DB '3 - Search by Category', 0DH, 0AH,
           DB 'Enter Search Option [1-3]: $'
...
TotalRevenue DW 0
ReportPINPrompt DB 0DH, 0AH, 'Enter PIN (4 digits): $'
...
AddToCart PROC
    ...
    ; Store item in cart (simplified - just increment count)
    MOV AL, CartCount
    INC AL
    MOV CartCount, AL
    
    ; Get price based on category and item
    CALL CalculatePrice
    ...
    ; Store last item price for potential removal
    MOV AX, CurrentPrice
...
RemoveLoop:
    ; Ask if user wants to remove last item
...
SkipRemove:
    ...
    ; Check discount eligibility (minimum 500 TK)
    CMP CartTotal, 500
    JL SkipDiscount
...
SearchByLetter:
    ...
    INT 21H
    MOV ItemNumber, AL
    ; Search for products starting with letter (raw input, not GetInput)
    CMP AL, 'R'
    JE ShowRice
    CMP AL, 'r'
    JE ShowRice
...
SearchByCategory:
    PrintString CategorySearchPrompt
    GetInput
    CMP AL, 1
